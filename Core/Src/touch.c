...
/* Pin assignment and initial calibration follow D:\git\2025g. */
...
#define TOUCH_MOSI_WRITE(x) \
  HAL_GPIO_WritePin(TOUCH_MOSI_PORT, TOUCH_MOSI_PIN, \
                    (x) ? GPIO_PIN_SET : GPIO_PIN_RESET)
#define TOUCH_CLK_WRITE(x) \
  HAL_GPIO_WritePin(TOUCH_CLK_PORT, TOUCH_CLK_PIN, \
                    (x) ? GPIO_PIN_SET : GPIO_PIN_RESET)
#define TOUCH_CS_WRITE(x) \
  HAL_GPIO_WritePin(TOUCH_CS_PORT, TOUCH_CS_PIN, \
                    (x) ? GPIO_PIN_SET : GPIO_PIN_RESET)
#define TOUCH_PEN_READ() \
  (HAL_GPIO_ReadPin(TOUCH_PEN_PORT, TOUCH_PEN_PIN) == GPIO_PIN_RESET)
#define TOUCH_MISO_READ() \
  (HAL_GPIO_ReadPin(TOUCH_MISO_PORT, TOUCH_MISO_PIN) == GPIO_PIN_SET)
...
static uint16_t Touch_MapToScreen(uint16_t raw,
                                  uint16_t raw_min,
                                  uint16_t raw_max,
                                  uint16_t screen_max,
                                  uint8_t invert);
...
uint8_t Touch_Scan(TouchState *state)
{
  ...
  state->raw_x = raw_x;
  state->raw_y = raw_y;
  state->x = Touch_MapToScreen(raw_x, TOUCH_RAW_X_MIN, TOUCH_RAW_X_MAX,
                               lcddev.width - 1U, TOUCH_INVERT_X);
  state->y = Touch_MapToScreen(raw_y, TOUCH_RAW_Y_MIN, TOUCH_RAW_Y_MAX,
                               lcddev.height - 1U, TOUCH_INVERT_Y);
  state->pressed = 1U;
  return 1U;
}
...
static uint16_t Touch_ReadFiltered(uint8_t cmd)
{
  ...
  for (uint8_t i = TOUCH_LOST_VAL;
       i < (TOUCH_READ_TIMES - TOUCH_LOST_VAL);
       ++i)
  {
    sum += values[i];
  }
  ...
  return (uint16_t)(sum /
                    (TOUCH_READ_TIMES - 2U * TOUCH_LOST_VAL));
}
                                  ...
                                  uint8_t invert)
{
  ...
  {
    value = ((uint32_t)(raw - raw_min) * screen_max) /
            (raw_max - raw_min);
  }
